...
void readSensors() {
    ...
    // Moving average for gas sensor to reduce noise
...
void sendSensorData() {
    ...
    if (httpResponseCode > 0) {
        ...
        Serial.println("Data sent to backend: " + response);
    } else {
        Serial.println("Backend error: " + String(httpResponseCode));
    }
    http.end();
}
...
void controlDevice(uint8_t * payload) {
    ...
    if (device == "light") {
        ...
        if (room == "living") {
            if (state == HIGH) 
                openTheDoor(doorLiving, livingDoorOpen, true, DOOR_LIVING);
            else 
                closeTheDoor(doorLiving, livingDoorOpen, false, DOOR_LIVING);
        } else if (room == "bedroom") {
            if (state == HIGH) 
                openTheDoor(doorBedroom, bedroomDoorOpen, true, DOOR_BEDROOM);
            else 
                closeTheDoor(doorBedroom, bedroomDoorOpen, false, DOOR_BEDROOM);
        } else if (room == "kitchen") {
            if (state == HIGH) 
                openTheDoor(doorKitchen, kitchenDoorOpen, true, DOOR_KITCHEN);
            else 
                closeTheDoor(doorKitchen, kitchenDoorOpen, false, DOOR_KITCHEN);
        }
    }
}
...
void handleRoot() {
    server.sendHeader("Access-Control-Allow-Origin", "*");
    server.send(200, "text/plain", "ESP32 Smart Home API (No MQTT)\nUse /resetwifi to clear WiFi settings");
}
